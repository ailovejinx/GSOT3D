...
def get_offset_box(box, offset, use_z=True, offset_max=[2.0, 2.0, 1.0], degree=True, is_training=True):
    ...
    if len(offset) == 3:
        ...
        if degree:
            new_box.rotate(
                Quaternion(axis=[0, 0, -1], degrees=offset[5]) *
                Quaternion(axis=[0, -1, 0], degrees=offset[4]) *
                Quaternion(axis=[-1, 0, 0], degrees=offset[3])
            )
        else:
            new_box.rotate(
                Quaternion(axis=[0, 0, -1], radians=offset[5]) *
                Quaternion(axis=[0, -1, 0], radians=offset[4]) *
                Quaternion(axis=[-1, 0, 0], radians=offset[3])
            )
    if is_training:
